...
use foundry_cli::{
    cmd::{
        forge::{cache::CacheSubcommands, cage::CageSubcommands, watch},
        Cmd,
    },
    handler,
...
fn main() -> eyre::Result<()> {
    ...
    match opts.sub {
        ...
        Subcommands::Cage(cmd) => match cmd.sub {
            CageSubcommands::Test(cmd) => {
                cmd.test()?;
            }
            CageSubcommands::RunTc(cmd) => {
                cmd.run_tc()?;
            }
            CageSubcommands::Analyze(cmd) => {
                cmd.analyze()?;
            }
            CageSubcommands::RunSol(cmd) => {
                cmd.run_sol()?;
            }
        },
