...
ethers::contract::abigen!(
    ...
    r#"[
            ...
            struct EVMCall {address to; bytes data; uint256 value}
            roll(uint256)
            warp(uint256)
            difficulty(uint256)
            fee(uint256)
            ...
            pauseGasMetering()
            resumeGasMetering()
            getRelevantTokenAddrs()(address[])
            getTargetAddrs()(address[])
            getBaseTokenAddr()(address)
            getPairAddr()(address)
            getRouterAddr()(address)
            updateTokenBalance(address, address, uint256)
            addRelevantTokenAddr(address)
            debug(string)
            initialize(address, address)
            notifyOutOfCall(uint8)
            replacePlaceholderValue(bytes)(bytes)
            getTargetTokenAddr()(address)
            saveBalanceSnapshot()
            checkInvariantBroken()
            notifyExploitSuccess(uint256)
            recordTransferToPairFee()
            hasNextCall()(bool)
            getNextCall()(EVMCall)
            notifyInitialSwapFailed()
            registerFee(uint256)
            getFee()(uint256)
            getInitialTokenPercent()(uint256)
            registerBurnAmount(uint)
    ]"#,
);
